...
function preload() {
	...
	soundscape = loadSound("assets/desktop.wav");
}
...
function draw() {
	...
	if (millis() > 2000) {
		...
		poem.forEach((currentLine, index) => {
			...
			if (currentLine.position < height + currentLine.lineHeight * 3 && currentLine.position > -currentLine.lineHeight) {
				...
				} else if (index > 195 && index < 303 || index > 78 && index < 84 || index > 109 && index < 117 || index > 428 && index < 433 || index > 602 && index < 612 || index > 591 && index < 595) {
				...
				// this is where the typing animation begins:
					...
					if (!scroll) {
						...
						if (index - 319 === lineIndex) {
							...
							indexSinceStart = Math.floor((millis() - typeAnimationStart) / 200);

							if (indexSinceStart <= currentLine.content.length) {
								text(currentLine.content.substring(0, indexSinceStart), 32, currentLine.position);
							} else {
								typeAnimationStart = millis();
								lineIndex++;
							}
